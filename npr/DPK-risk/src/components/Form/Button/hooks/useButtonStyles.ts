import type {ButtonClassNameType, ButtonProps, ButtonSizesType} from "../ButtonTypes";
...
function useButtonStyles(
  ...
  const buttonStylesSizes: Record<ButtonSizesType, string> = {
    ...
    'md': !justIcon ? `px-4 py-2 text-base` : 'p-2.5',
  }

  const buttonStyles: ButtonClassNameType = {
    default: `flex items-center justify-center font-medium duration-200 select-none relative overflow-hidden
     whitespace-nowrap  disabled:cursor-not-allowed ${readOnly ? 'cursor-default' : ''}
