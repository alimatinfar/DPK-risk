...
const ExpandableTable: FC<IExpandableTableProps> = ({
  ...
  const [visibleColumns, setVisibleColumns] = useState(
    filter
      ? originalColumns.slice(0, 6).map((col) => col.field)
      : originalColumns.map((col) => col.field)
  );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !rowsPerPageRef.current.contains(event.target as Node)
      ) {
  ...
  const columns = filter
    ? originalColumns.filter((col) => visibleColumns.includes(col.field))
    : originalColumns;
  ...
  return (
    <div className={`bg-default shadow rounded-lg  ${className}`}>
      <table className={`w-full table-auto  ${tblClassName}`}>
        <thead className="bg-primary-100">
          <tr className="text-sm">
            ...
            {expandedRowTemplate && !filter && (
              <th className="w-12 font-normal p-3 rounded-tr-lg"></th>
            )}
            {showRowNumber && (
              <th
                className={`w-12 font-normal p-3 text-center tracking-wider ${
                  !filter && "rounded-tr-lg"
                }`}
              >
                ردیف
              </th>
            ...
            {columns.map((col, index) => (
              <th
                key={col.field}
                className={`p-3 font-normal text-center tracking-wider ${
      ...
      {paginator && (
        <div className="px-3 py-1 flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            <div className="text-xs text-gray-700 dark:text-gray-300 whitespace-nowrap">
              نمایش {(currentPage - 1) * rowsPerPage + 1} تا{" "}
              {Math.min(currentPage * rowsPerPage, filteredData.length)} از{" "}
            ...
            <div className="relative" ref={rowsPerPageRef}>
              <button
                onClick={() =>
                  setShowRowsPerPageDropdown(!showRowsPerPageDropdown)
                }
                className="flex items-center text-xs text-gray-700 dark:text-gray-300 px-1 py-1 border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                aria-label="تعداد ردیف‌های هر صفحه"
              >
                {rowsPerPage} ردیف در صفحه
                <FiChevronDown
                  className={`mr-2 w-3 h-3 transition-transform ${
                    showRowsPerPageDropdown ? "transform rotate-180" : ""
                  }`}
                />
              </button>

              {showRowsPerPageDropdown && (
                <div className="absolute bottom-full mb-2 left-0 w-10 bg-default rounded-md shadow-lg z-10 border border-primary">
                  {rowsPerPageOptions.map((option) => (
                    <button
                      key={option}
                      onClick={() => handleRowsPerPageChange(option)}
                      className={`w-full text-center  text-xs  ${
                        rowsPerPage === option ? " text-primary" : ""
                      }`}
                    >
                      {option}
                    </button>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-1 py-1 rounded-md border border-gray-300 dark:border-gray-600 text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              قبلی
            </button>
            <button
