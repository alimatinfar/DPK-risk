...
function InputErrorMessage({errorMessage, description, endErrorMessage}: Props) {
  ...
  const errorMessageElement = useMemo(function () {
    ...
    return endErrorMessage ? (
      <div className={`${className} flex items-center justify-between space-x-reverse space-x-1 `}>
        <span>{text}</span>
        {endErrorMessage}
      </div>
      ) : (
  ...
  }, [errorMessage, description, wrapperClass])

  return errorMessage ? (
    <div className={`${wrapperClass} flex items-center space-x-1 space-x-reverse`}>
      <ErrorIcon />
      {errorMessageElement}
    </div>
  ) : errorMessageElement
